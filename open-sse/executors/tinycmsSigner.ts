...
// NOTE (OmniRoute): upstream wasm-bindgen glue polyfilled `encodeInto` here for
// legacy browsers. Node always provides it, so the branch is unreachable — and
// TS narrows the encoder to `never` inside it, erroring on the assignment.
...
async function __wbg_init(module_or_path) {
  ...
  // NOTE (OmniRoute): upstream wasm-bindgen glue defaulted to
  // `new URL('wasm_signer_bg.wasm', import.meta.url)` here. That branch is
  // unreachable on this server — initTinyCmsWasm() always passes the embedded
  // WASM_BASE64 buffer — but webpack statically resolves the URL at build time
  // and fails the build with "Can't resolve 'wasm_signer_bg.wasm'". The binary
  // ships inlined, so there is no sidecar file to point at.
  if (module_or_path === undefined) {
    throw new Error(
      "TinyCMS WASM must be initialized via initTinyCmsWasm(); no sidecar wasm_signer_bg.wasm exists."
    );
  }
  const imports = __wbg_get_imports();
