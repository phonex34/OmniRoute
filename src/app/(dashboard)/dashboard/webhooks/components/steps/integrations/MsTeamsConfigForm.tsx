...
interface MsTeamsConfigFormProps {
  value: MsTeamsConfig;
  onChange: (v: MsTeamsConfig) => void;
  t: (key: string) => string;
}

type UrlState = "idle" | "checking" | "ok" | "blocked" | "invalid";

export function MsTeamsConfigForm({ value, onChange, t }: MsTeamsConfigFormProps) {
    ...
    <div className="space-y-4">
      <div>
        <label className="text-xs font-medium uppercase tracking-wider text-text-muted">
          {t("msteams.webhookUrl")}
        </label>
        <input
          value={value.webhookUrl}
          onChange={(e) => onChange({ webhookUrl: e.target.value })}
          placeholder={t("msteams.webhookUrlPlaceholder")}
        ...
        <p className="mt-1 text-xs text-text-muted">{t("msteams.webhookUrlHint")}</p>
      </div>
